...
In this lesson we will study how to design filters

So we can choose between two types of filters: recursive (that may have infinite impulse response) and FIR filters
...
The impulse response of a FIR filter is composed of a finite number $N$ of non-zero coefficients.

We assume here the case of **real and causal**

Then, the frequency response can be factorized in a real continuous function $H_R$ multiplying a complex **linear phase component** of the continuous variable $\nu$.
...
If the input signal has its frequency support in the passband, then the waveform of the output is similar to the one of the input: 
...
- the possibility of synthesizing them with exact linear phase in frequency, 
...
- meaning that these errors do not propagate. 
- In the case of recursive filters instead, errors due to finite precision may propagate and cause instabilities if the poles are near the unit circle
...
We have the Type I and Type II filters that 
...
These filters feature a zero in the frequency response at $\nu=0.5$, so the highest frequency of the passband is fixed at $\nu=0.45$
...
These methods consist in defining some properties of the desired filters

and optimize a criterion, called the Chebyshev error criterion, to estimate the filter parameters.
...
- the synthesis is computationally demanding (that is, computing the filter)
...
We want to optimize a FIR filter of length $N$ whose frequency response is given by 
$$ H(\eipiv) = e^{- i 2 \pi \nu \frac{N - 1}{2}} H_R(\nu)$$

The optimization criterion is based on the error which
...
Then the Remez exchange algorithm empirically searches for a solution.
...
The filter has a zero on the unit circle, and its frequency response presents a cancellation at $\nu_k$

We can leverage this fact to create stopbands by setting the $\nu_k$ in the stopband
...
Then by building the filter as this ratio, we can see that we obtain a flattening of the passband - but a non-linear phase in the neighborhood of $\nu=0.085$, which is the frequency of the pole
...
Generally we find the poles in the vicinity of the transition band of the filter, and the cutoff will be even more selective the closer the poles are to the unit circle.

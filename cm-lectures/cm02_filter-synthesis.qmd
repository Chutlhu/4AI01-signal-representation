...
We are looking for a filter $h$ which is
...
To implement nicely these two conditions, we need a trick.
...
A function of period 2 can be considered as a function of period 1 on a double grid, and it is a legitimate Fourier transform: 
...
- the half-sample shift becomes a shift of $p$ samples, 
- and the relation reads $H(z^2) = z^{-p}G(z)$.

In the time domain: $H(z^2)$ is the transform of $h$ with a zero inserted between every pair of samples. So $g$, delayed by $p$ samples, is the version of $h$ stuffed with zeros.

Now, the length of $g$ is $2N-1$, which is odd: an odd length always has a center, so the symmetry is finally expressible.
...
![](images/continuous_to_discrete_adc_comparison.svg)


::: {.notes}

Let conclude with one final question: can a digital filter reproduce exactly what an analog filter does? 

- we have here two path:
  - one analog, top branch, with analog filter $H(f)$, than sampled
  - and one digital, bottom part with digital filter $H_e(f)$ who receive a sampled signal
- then we ask, is the output zero?

This is true only if the filters are truly bandlimited and there is no aliasing, but you will see this later in the course
